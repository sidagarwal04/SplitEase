  ...
  try {
    ...
    if (!apiKey) {
      return json(200, {
        added: false,
        emailSent: false,
        warning: 'RESEND_API_KEY not configured; invite not actually sent.',
      });
    }
    ...
    return json(200, { added: false, emailSent: true });
  } catch (err) {
    return json(err.status || 500, { error: err.message });
  }
};

function inviteEmail({ groupName, inviterName, appUrl }) {
  return `<!doctype html>
